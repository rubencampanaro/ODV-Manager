...
function elencoAzioni($giornata) {
  ...
  WHERE VA.azione_volontariato = AV.codice AND (VA.data_inizio <= '$giornata' AND VA.data_fine >= '$giornata') AND VA.tabella = 'amministrazione';";
...
if(isSet($_POST['opzione']) && isSet($_POST['dati']))
{
  switch ($_POST['opzione']) {
    ...
    case "form_dettagli":
      ...
      case "dettagli":
      $query1 = "SELECT VA.codice, AV.nome as nome_azione, AV.descrizione as descrizione_azione, AM.codice as codice_utente, AM.nome, AM.cognome, AM.codice_fiscale,
      VA.data_inizio, VA.data_fine, VA.ora_inizio, VA.ora_fine
      FROM volontari_azioni as VA, azioni_volontariato as AV, amministrazione as AM
      WHERE VA.codice = ".$_POST['dati']." AND VA.azione_volontariato = AV.codice AND VA.volontario = AM.codice;";
