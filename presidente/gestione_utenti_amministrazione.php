...
function elencoUtenti() {
  $query = "SELECT codice, nome, cognome, codice_fiscale, luogo_nascita, date_format(data_nascita,'%d/%m/%Y') AS data_nascita FROM amministrazione";
  ...
  if (empty($output))
  $output = '<tr><td class="text-center" colspan="100%">Nessun utente amministrazione registrato.</td></tr>';
...
<head>
  ...
  function opzioneUtente(element, opzione, codice) {
    ...
    httpRequest.open("POST", "./app.php?n=functions_utente_amministrazione.php", true);
...
<body>
  ...
  <div class="py-5 text-center bg-danger">
    <div class="d-flex align-items-center justify-content-center">
      ...
      <h1 class="text-white fw-bold text-shadow">Gestione utenti - Amministrazione</h1>
    </div>
  </div>
