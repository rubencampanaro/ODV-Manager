...
function compilaBenemerenze($codice_volontario) {
  $query = "SELECT codice, nome
  FROM benemerenze
  WHERE codice <> ALL
  (SELECT benemerenza
    FROM volontari_benemerenze
    WHERE volontario = $codice_volontario AND tabella = 'volontari');";
    $risultato = connectDB($query);
    $output = "<select id='benemerenza' form='form_benemerenza' required class='form-select'>
  ...
  function visualizzaBenemerenze($codice_volontario) {
    ...
    WHERE VB.volontario = ".$codice_volontario." AND VB.benemerenza = BN.codice AND tabella = 'volontari';";
    $risultato = connectDB($query);
    $output = "";
    while($riga = mysqli_fetch_assoc($risultato))
    {
      ...
      $output .= '<td><button class="btn btn-danger btn-sm" onclick="'."gestioneBenemerenze(this, 'elimina', '".$codice_volontario."|".$codice_benemerenza."')".'">Elimina</button></td>';
      $output .= "</tr>";
    }
    mysqli_free_result($risultato);
    $output .= '<tr>
    <form action="#" id="form_benemerenza"
    onsubmit="'."gestioneBenemerenze(this, 'inserisci', $codice_volontario)".'"
    method="post"></form>
    <td class="text-center" colspan="2">'.compilaBenemerenze($codice_volontario) . '</td>
  ...
  if(isSet($_POST['opzione']) && isSet($_POST['dati']))
  {
    switch ($_POST['opzione']) {
      ...
      $query = "INSERT INTO volontari_benemerenze VALUES (0, ".$dati[0].", 'volontari', ".$dati[1].", '".$dati[2]."');";
      if (connectDB($query))
