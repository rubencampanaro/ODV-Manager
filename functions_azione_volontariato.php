...
function elencoAzioni($giornata) {
  ...
  WHERE VA.azione_volontariato = AV.codice AND volontario = ".$_SESSION['codice']." AND tabella = '".$_SESSION['tabella']."'";
  if (!empty($giornata))
  $query .= " AND (VA.data_inizio <= '$giornata' AND VA.data_fine >= '$giornata')";
  $query .= ";";
  ...
  while ($riga = mysqli_fetch_assoc($risultato)) {
    ...
    $output .= '<td><button type="button" class="btn btn-success btn-sm mb-1 mb-lg-0" data-bs-toggle="modal" data-bs-target="#dettagli" onclick="'."opzioneAzione(this, 'dettagli', ".$riga['codice'].")".'">Dettagli</button>
    <button type="button" class="btn btn-danger btn-sm" onclick="'."opzioneAzione(this, 'elimina', ".$riga['codice'].")".'">Elimina</button></td>';
    $output .= "</tr>";
  }
  if (empty($output))
...
if(isSet($_POST['opzione']) && isSet($_POST['dati']))
{
  switch ($_POST['opzione']) {
    ...
    case "form_dettagli":
      ...
      case "dettagli":
      $query1 = "SELECT VA.codice, AV.nome, AV.descrizione, VA.data_inizio, VA.data_fine, VA.ora_inizio, VA.ora_fine
      FROM volontari_azioni as VA, azioni_volontariato as AV
      WHERE VA.codice = ".$_POST['dati']." AND VA.azione_volontariato = AV.codice;";
      ...
      case "elimina":
      $query1 = "DELETE FROM volontari_azioni_dettagli WHERE azione_volontariato=".$_POST['dati'].";";
      $query2 = "DELETE FROM volontari_azioni WHERE codice=".$_POST['dati'].";";
      $query = $query1 . " " . $query2;
